...
if ($("body").hasClass("main"))
	$(window).on("scroll touchmove load", e => {
		$(".main > div[id]").each((i, el) => {
			let $this = $(el),
				fix = 0;

			if (!$("a.scroll[href='#"+$this.attr("id")+"']").length || $("a.scroll[href='#"+$this.attr("id")+"']").hasClass("active"))
				return

			if ($("a.scroll[href='#"+$this.attr("id")+"']").closest("li").is($("a.scroll[href='#"+$this.attr("id")+"']").closest("ul").find("li:last-child")))
				fix = parseInt($this.css("margin-top"));
...
if ($(".clients-counter__number").length)
...
if (X.IS_MAIN){
	$(".promo__cont").mousemove(e => {
		...
		let x = ((e.pageX - $(window).width()/2) * -1 / 20), 
...
$(".directions__slider").each((i, el) => {
	...
	$slider.on("beforeChange", (e, slick, curSlide, nextSlide) => {
		...
		$(".directions-menu").find("li:eq("+nextSlide+")")
			.find(".directions-menu__link").addClass("active");
	});
...
if ($("select.selectize").length && X.IS_DESKTOP)
...
X.inner(() => {
	...
	let $slider = $(".creating__slider-slider").slick({
		slidesToShow: 1,
		slidesToScroll: 1,
	...
	$(".creating-nav__el").click(function(){
		...
		if (!$(".creating-nav").hasClass("js__opened") 
			&& $(window).width() <= 660){
	...
	if (!X.IS_DESKTOP && $(".timeline__number").length)
		$(".timeline__number").countTo()


});

if (X.IS_DESKTOP)
	$(".text-page a:not([class]), .footer-mail a:not([class]),\
	 .head-phone__links a:not([class]), .direction__links a,\
	  .direction__title a").each((i, el) => {
		let $this = $(el);
...
document.addEventListener("touchmove", function(e){
	if ($("body").hasClass("js__mobile-menu-opened") 
		&& $(window).width() <= 1000) {
		e.preventDefault();
...
const setImgHeight = e => {
	$(".direction__img").height("auto");

	$(".direction__img").height(Math.max(...$(".direction__img").map(function(){
		return $(this).height()
	})));
}
